...
namespace wServer.networking.handlers
{
    internal class InvSwapPacketHandler : PacketHandlerBase<InvSwapPacket>
    {
        ...
        private void dropBag(Client client, Item item, ItemData data)
        {
            Container con = new Container(client.Manager, 0x0503, (1000 * 60), true)
            {
                BagOwners = new[] {client.Account.AccountId}
            };
            con.Inventory[0] = item;
            con.Inventory.Data[0] = data;
            con.Move(client.Player.X + (float)((invRand.NextDouble() * 2 - 1) * 0.5),
                client.Player.Y + (float)((invRand.NextDouble() * 2 - 1) * 0.5));
            con.Size = 75;
            client.Player.Owner.EnterWorld(con);

            client.Player.Client.SendPacket(new InvResultPacket { Result = 0 });
        }
    }
}