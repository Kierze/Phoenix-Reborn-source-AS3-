...
namespace wServer.realm.entities
{
    ...
    public class Projectile : Entity
    {
        ...
        public int penetrants { get; set; }
        public ActivateAbility abil { get; set; }
        //public int CondChance { get; set; }
        //public int CondEffect { get; set; }
        ...
        private bool TickCore(long elapsedTicks, RealmTime time)
        {
            ...
            foreach (Entity i in Owner.EnemiesCollision.HitTest(pos.X, pos.Y, 2))
            {
                if (i == ProjectileOwner.Self) continue;
                if (i is Container) continue;
                if (hitted.Contains(i)) continue;
                if (!(i is ItemEntity)) continue;
                double xSide = (i.X - pos.X) * (i.X - pos.X);
                double ySide = (i.Y - pos.Y) * (i.Y - pos.Y);
                if (xSide <= 0.5 * 0.5 && ySide <= 0.5 * 0.5 && xSide + ySide <= nearestRadius)
                {
