...
namespace wServer.logic.behaviors
{
    internal class Grenade : Behavior
    {
        ...
        public Grenade(double radius, int damage, int penetration, double range = 5,
                ...
                if (player != null || fixedAngle != null)
                {
                    ...
                    host.Owner.Timers.Add(new WorldTimer(1500, (world, t) =>
                    {
                        ...
                        world.AOE(target, radius, true, p => {
                            if (p is Player)
                            {
                                (p as IPlayer).Damage(damage, penetration, host as Character);
                            }
                            else
                            {
                                (p as ItemEntity).Damage(damage, penetration, host as Character);
                            }
                        });
                    }));
                }
