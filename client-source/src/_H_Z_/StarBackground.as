...
public class StarBackground extends Background {
    ...
    private function _lh():void {
        var _local3:Star;
        var _local1:AssetFile = AssetLibrary.getAssetFile("stars");
        var _local2:Star = new Star(((Math.random() * 1000) - 500), ((Math.random() * 1000) - 500), (4 * (0.5 + (0.5 * Math.random()))), _local1.Bitmaps[int((_local1.Bitmaps.length * Math.random()))]);
        for each (_local3 in this._P_j) {
