package com.company.PhoenixRealmClient.objects {
import ParticleAnimations.XmlEffectProperties;

import com.company.PhoenixRealmClient.appengine.RemoteTexture;
import com.company.PhoenixRealmClient.parameters.Parameters;
import com.company.PhoenixRealmClient.util.AnimatedTextures;
import com.company.PhoenixRealmClient.util.MaskedBitmap;
import com.company.PhoenixRealmClient.util._lJ_;
import com.company.util.AssetLibrary;

import flash.display.BitmapData;
import flash.utils.Dictionary;

public class TextureFromXML {

        public function TextureFromXML(_arg1:XML) {
            var _local2:XML;
            super();
            if (_arg1.hasOwnProperty("Texture"))
            {
                this.parse(XML(_arg1.Texture));
            }
            else
            {
                if (_arg1.hasOwnProperty("AnimatedTexture"))
                {
                    this.parse(XML(_arg1.AnimatedTexture));
                }
                else
                {
                    if (_arg1.hasOwnProperty("RemoteTexture"))
                    {
                        this.parse(XML(_arg1.RemoteTexture));
                    }
                    else
                    {
                        if (_arg1.hasOwnProperty("RandomTexture"))
                        {
                            this.parse(XML(_arg1.RandomTexture));
                        }
                        else
                        {
                            this.parse(_arg1);
                        }
                    }
                }
            }
            for each (_local2 in _arg1.AltTexture) {
                this.parse(_local2);
            }
            if (_arg1.hasOwnProperty("Mask")) {
                this.parse(XML(_arg1.Mask));
            }
            if (_arg1.hasOwnProperty("Effect")) {
                this.parse(XML(_arg1.Effect));
            }
        }
        public var texture_:BitmapData = null;
        public var mask_:BitmapData = null;
        public var _yN_:_lJ_ = null;
        public var _07_:Vector.<TextureFromXML> = null;
        public var _V_N_:Dictionary = null;
        public var _0Y_:int;
        public var _0L_G_:XmlEffectProperties = null;

        public function getTexture(_arg1:int = 0):BitmapData {
            if (this._07_ == null) {
                return (this.texture_);
            }
            var _local2:TextureFromXML = this._07_[(_arg1 % this._07_.length)];
            return (_local2.getTexture(_arg1));
        }

        public function _0K_J_(_arg1:int):TextureFromXML {
            if (this._V_N_ == null) {
                return (null);
            }
            return (this._V_N_[_arg1]);
        }

        private function parse(_arg1:XML):void {
            var _local2:MaskedBitmap;
            var _local3:RemoteTexture;
            var _local4:XML;
            switch (_arg1.name().toString()) {
                case "Texture":
                    this.texture_ = AssetLibrary.getBitmapFromFileIndex(String(_arg1.File), int(_arg1.Index));
                    return;
                case "Mask":
                    this.mask_ = AssetLibrary.getBitmapFromFileIndex(String(_arg1.File), int(_arg1.Index));
                    return;
                case "Effect":
                    this._0L_G_ = new XmlEffectProperties(_arg1);
                    return;
                case "AnimatedTexture":
                    this._yN_ = AnimatedTextures._J_v(String(_arg1.File), int(_arg1.Index));
                    _local2 = this._yN_._l8(0, _lJ_._sS_, 0);
                    this.texture_ = _local2.image_;
                    this.mask_ = _local2.mask_;
                    return;
                case "RemoteTexture":
                    this.texture_ = AssetLibrary.getBitmapFromFileIndex("lofiObj3", 0xFF);
                    if (Parameters.domainCheck()) {
                        _local3 = new RemoteTexture(_arg1.Id, _arg1.Instance, this._Z_v);
                        _local3.run();
                    }
                    this._0Y_ = ((_arg1.hasOwnProperty("Right")) ? _lJ_.RIGHT : _lJ_.DOWN);
                    return;
                case "RandomTexture":
                    this._07_ = new Vector.<TextureFromXML>();
                    for each (_local4 in _arg1.children()) {
                        this._07_.push(new TextureFromXML(_local4));
                    }
                    return;
                case "AltTexture":
                    if (this._V_N_ == null) {
                        this._V_N_ = new Dictionary();
                    }
                    this._V_N_[int(_arg1.@id)] = new TextureFromXML(_arg1);
                    return;
            }
        }

        private function _Z_v(_arg1:BitmapData):void {
            if (_arg1.width > 16) {
                AnimatedTextures.add("remoteTexture", _arg1, null, (_arg1.width / 7), _arg1.height, _arg1.width, _arg1.height, this._0Y_);
                this._yN_ = AnimatedTextures._J_v("remoteTexture", 0);
                this.texture_ = this._yN_._l8(0, _lJ_._sS_, 0).image_;
            } else {
                this.texture_ = _arg1;
            }
        }

    }
}

