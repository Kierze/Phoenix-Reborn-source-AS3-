...
public class _qQ_ {
    ...
    private function _U_X_():void {
        Parameters.ClientSaveData.charIdUseMap[this.data.charId] = new Date().time;
        Parameters.save();
    }
    ...
    private function _0J_t():int {
        if (Parameters.ClientSaveData.needsTutorial) {
            return (Parameters.TUT_ID);
        }
        if (Parameters.ClientSaveData.needsRandomRealm) {
            return (Parameters.RAND_REALM);
        }
        if (Parameters.ClientSaveData.needsNexusTutorial) {
            Parameters.ClientSaveData.needsNexusTutorial = false;
            Parameters.save();
